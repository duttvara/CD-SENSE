...
resampled_data['Marsh'] = resampled_data['Marsh'].astype(int)
print(resampled_data['Marsh'].dtype)
print("Resampled 'Marsh' distribution:")
...
# Apply the function to categorize the Marsh scores
resampled_data['Marsh_Category'] = resampled_data['Marsh'].apply(categorize_marsh)

# Now define X and y using the new 'Marsh_Category'
X = resampled_data[feature_cols]
y = resampled_data['Marsh_Category']
resampled_data['Marsh_Category'].head()
# Define the mapping for Marsh_Category
category_mapping = {'Normal': 0, 'Mild': 1, 'Severe': 2}

# Apply the mapping to convert the categories into numerical values
resampled_data['Marsh_Category'] = resampled_data['Marsh_Category'].map(category_mapping)

# Check if the changes are applied correctly
print(resampled_data['Marsh_Category'].tail())
...
X = resampled_data[feature_cols]
y = resampled_data['Marsh_Category']
resampled_data['Marsh_Category'] = resampled_data['Marsh_Category'].astype(int)
...
models = {
    ...
    'Random Forest': RandomForestClassifier(max_depth= 20, max_features='sqrt', min_samples_split= 5, n_estimators=50),
    'Extra Trees': ExtraTreesClassifier(max_depth=None, max_features='sqrt', min_samples_split=2, n_estimators=200),
    'Decision Trees': DecisionTreeClassifier(max_depth=30, max_features='sqrt', min_samples_split=2),
    'Logistic Regression': LogisticRegression(C=10, max_iter=100, penalty='l1', solver='saga'),
    'AdaBoost': AdaBoostClassifier(learning_rate=1, n_estimators=200),
    'XGBoost': XGBClassifier(learning_rate=0.2, max_depth=4, n_estimators=200, subsample=1.0),
...
X = resampled_data[feature_cols]  # Feature columns
y = resampled_data['Marsh_Category']        # Multi-class target column with 5 classes

# Split data into training and validation sets
train_X, val_X, train_y, val_y = train_test_split(X, y, random_state=1, stratify=y)

my_model = XGBClassifier(learning_rate=0.2, max_depth=4, n_estimators=200, subsample=1.0)
